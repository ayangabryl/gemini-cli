...
function isBenchmarkAddress(addr: ipaddr.IPv4 | ipaddr.IPv6): boolean {
  return (
    ...
    addr.match(IANA_BENCHMARK_RANGE[0] as ipaddr.IPv4, IANA_BENCHMARK_RANGE[1])
  );
}
 ...
 */
function handleFetchError(error: unknown, url: string | URL | Request): never {
  const urlString =
